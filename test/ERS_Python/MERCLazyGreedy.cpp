...
// if compute_similarity > 0 means input is gradient and we need to compute similarity here
// if compute_similarity == 0 then we don't need to compute similarity here
MERCDisjointSet* MERCLazyGreedy::ClusteringTree(int nVertices,MERCInput &edges,int kernel,double sigma,double lambda,int nC, int compute_similarity)
{
